...
function sendUpdateUserAJAX(row) {
    ...
    if (password != '-') parameters.password = loginEncrypted_Encrypt(password);
    ...
    ajaxHandler.addParams({
        module: 'API',
        format: 'json',
        method: 'UsersManagerEncrypted.updateUser'
    }, 'GET');
...
function sendDeleteUserAJAX(login) {
    ...
    ajaxHandler.addParams({
        module: 'API',
        format: 'json',
        method: 'UsersManagerEncrypted.deleteUser'
    }, 'GET');
...
function sendAddUserAJAX(row) {
    ...
    parameters.password = loginEncrypted_Encrypt($(row).find('input#useradd_password').val());
    ...
    ajaxHandler.addParams({
        module: 'API',
        format: 'json',
        method: 'UsersManagerEncrypted.addUser'
    }, 'GET');
    ajaxHandler.addParams(parameters, 'POST');
    ajaxHandler.redirectOnSuccess();
    ajaxHandler.setLoadingElement('#ajaxLoadingUsersManagement');
...
function sendUpdateUserAccess(login, access, successCallback) {
    ...
    ajaxHandler.addParams({
        module: 'API',
        format: 'json',
        method: 'UsersManagerEncrypted.setUserAccess'
    }, 'GET');
    ajaxHandler.addParams(parameters, 'POST');
...
function updateSuperUserAccess(login, hasSuperUserAccess)
{
    ...
    ajaxHandler.addParams({
        module: 'API',
        format: 'json',
        method: 'UsersManagerEncrypted.setSuperUserAccess'
    }, 'GET');
    ajaxHandler.addParams(parameters, 'POST');
